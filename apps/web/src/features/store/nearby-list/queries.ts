...
import { DEFAULT_LAT, DEFAULT_LNG, getStores } from '@/features/store/search/api';
...
export const NEARBY_STORES_QUERY_KEY = ['stores', 'nearby'] as const;
...
export function useNearbyStores({ lat, lng, limit, enabled = true }: UseNearbyStoresParams = {}) {
    const resolvedLat = lat ?? DEFAULT_LAT;
    const resolvedLng = lng ?? DEFAULT_LNG;

    return useInfiniteQuery({
        queryKey: [...NEARBY_STORES_QUERY_KEY, { lat: resolvedLat, lng: resolvedLng, limit }],
        queryFn: ({ pageParam }) =>
            getStores({
                lat: resolvedLat,
                lng: resolvedLng,
