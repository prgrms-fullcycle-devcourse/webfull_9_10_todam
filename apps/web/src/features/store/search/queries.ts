...
import { DEFAULT_LAT, DEFAULT_LNG, getAutocomplete, getStores } from './api';

export const STORES_SEARCH_QUERY_KEY = ['stores', 'search'] as const;
export const AUTOCOMPLETE_QUERY_KEY = ['stores', 'autocomplete'] as const;

const STORES_DEFAULT_LIMIT = 10;

export type UseStoreSearchParams = {
    keyword: string;
    lat?: number;
    lng?: number;
    limit?: number;
...
export function useStoreSearch({ keyword, lat, lng, limit, enabled = true }: UseStoreSearchParams) {
    const resolvedLat = lat ?? DEFAULT_LAT;
    const resolvedLng = lng ?? DEFAULT_LNG;

    return useInfiniteQuery({
        queryKey: [
            ...STORES_SEARCH_QUERY_KEY,
