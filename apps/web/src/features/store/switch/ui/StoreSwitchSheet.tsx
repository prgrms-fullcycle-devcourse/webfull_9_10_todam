...
export interface StoreSwitchSheetStore {
    id: string;
    name: string;
    status: StoreStatus;
...
export interface StoreSwitchSheetProps {
    stores: StoreSwitchSheetStore[];
    onSelect: (store: StoreSwitchSheetStore) => void;
    onRegister: () => void;
    onClose: () => void;
}
...
export function StoreSwitchSheet({ stores, onSelect, onRegister, onClose }: StoreSwitchSheetProps) {
    return (
        <StandardBottomSheet
            title="공방을 전환하시겠어요?"
