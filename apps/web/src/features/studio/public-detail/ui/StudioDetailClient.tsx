...
import { ConvenienceChips } from '@/entities/studio/ui/ConvenienceChips';
import { StudioImageCarousel } from '@/entities/studio/ui/StudioImageCarousel';
import { StudioInfoSummary } from '@/entities/studio/ui/StudioInfoSummary';
import { StudioLocation } from '@/entities/studio/ui/StudioLocation';
...
import { getStudioReviews } from '@/features/studio/reviews/api';
import { ApiError } from '@/shared/api';
import { useHeaderOverride } from '@/shared/lib/useHeaderOverride';
import { useToast } from '@/shared/model';

import { retryExceptNotFound, usePublicStudioDetail, usePublicStudioPrograms } from '../queries';

// 리뷰 미리보기 쿼리 (limit=3, sort=latest — Contract C)
function useStudioReviewsPreview(slug: string) {
    return useQuery({
        queryKey: ['public', 'studios', slug, 'reviews-preview'],
        queryFn: () => getStudioReviews(slug, { limit: 3, sort: 'latest' }),
...
export function StudioDetailClient() {
    ...
    const reviewsQuery = useStudioReviewsPreview(slug);
                        ...
                        <div className="flex flex-col gap-3">
                            {programs.map((program) => {
                                ...
                                return (
                                    <Link
                                        key={program.id}
                                        href={`/studio/${slug}/programs/${program.id}`}
                                        className="block"
                                    >
                                        <ClassItem
