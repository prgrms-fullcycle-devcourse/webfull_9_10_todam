...
import { useStudioReviewsPreview } from '@/features/studio/reviews';
...
export function StudioReviewPreview({ slug }: { slug: string }) {
    const router = useRouter();
    const { data, isLoading } = useStudioReviewsPreview(slug);
    ...
    return (
        <div className="flex snap-x snap-mandatory gap-2 overflow-x-auto py-2">
            ...
            <button
                type="button"
                onClick={() => router.push(`/studio/${slug}/reviews`)}
