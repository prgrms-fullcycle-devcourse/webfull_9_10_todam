...
- UI: <!-- 구현된 화면, 컴포넌트 -->
- 연동: `ReservationModule`을 `AppModule`에 등록하고 route snapshot에 파트너 예약 라우트 8개를 추가. 검증: `corepack pnpm --filter @todam/api typecheck`, `corepack pnpm --filter @todam/api test -- api-routes.snapshot.spec.ts`.
